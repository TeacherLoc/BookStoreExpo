...
const Home = ({ navigation }) => {
    ...
    return (
      <View style={{ flex: 1, paddingLeft: SIZES.padding }}>
        <FlatList
          ...
          showsHorizontalScrollIndicator={false}
          renderItem={renderItem}
          keyExtractor={(item) => `category-header-${item.id}`}
          horizontal
        />
      </View>
    );
  ...
  const PageHeader = () => (
    <>
      ...
      {/* Categories Section Header Part */}
      <View style={{ marginTop: SIZES.padding }}>{renderCategoryHeader()}</View>
      {/* Thêm khoảng trống trước khi danh sách sách theo danh mục bắt đầu */}
      <View style={{ marginTop: SIZES.radius }} />
    </>
  );

  const renderBookItemByCategory = ({ item }) => {
    return (
      <View
        style={{
        ...
        {/* Bookmark Button */}
        <TouchableOpacity
          style={{ position: "absolute", top: 5, right: SIZES.padding + 15 }}
  ...
  var booksForCategoryList = [];
  let selectedCategoryBooks = categories.filter(
    (a) => a.id == selectedCategory
  );
  if (selectedCategoryBooks.length > 0) {
    booksForCategoryList = selectedCategoryBooks[0].books;
  }

  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.black }}>
      <FlatList
        data={booksForCategoryList}
        renderItem={renderBookItemByCategory}
        keyExtractor={(item) => `category-book-${item.id}`}
